...
export const DailyFollowUpsFileSearch = () => {
    ...
    const filteredPeriodlist = [
        { name: { en: 'Current Week', ar: 'الاسبوع الجاري', code: 'CW' } },
        { name: { en: 'Current Month', ar: 'الشهر الجاري', code: 'CM' } },
    ]
    ...
    const ddl3Change = (e) => {
        setValue("searchByNoNameFile3", e.value);
        Storage.session.set("DashboardSelectedPersonID", e.value?.code);
        if (e.value?.code) {
            handleSearchNavigation(false, true);
        }
    ...
    return (
        <div className="row file-search">
            <DropDownComponent
                ...
                placeholder={translate('dailyFollowUps.mainFileName')}
                className="col-md-12 col-lg-4"
                filter={true}
                label={translate('dailyFollowUps.mainFileName')}
            />
            ...
            <DropDownComponent
                ...
                placeholder={translate('dailyFollowUps.defendantName')}
                className="col-md-12 col-lg-4"
                filter={true}
                label={translate('dailyFollowUps.defendantName')}
            />

            <DropDownComponent
                id="searchByNoNameFile-id3"
                name="searchByNoNameFile3"
                register={register}
                watch={watch}
                setValueMethod={setValue}
                options={filteredPeriodlist}
                optionLabel={`name.${$lang}`}
                errors={errors}
                onChange={(e) => ddl3Change(e)}
                placeholder={translate('dailyFollowUps.choosePeriod')}
                className="col-md-12 col-lg-4"
                filter={true}
                label={translate('dailyFollowUps.choosePeriod')}
            />
        </div>
    );
};
